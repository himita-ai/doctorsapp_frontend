...
const Inventory = () => {
  ...
  const getInventoryData = async () => {
    try {
      ...
      const response = await axios.get(`mongodb+srv://himitagangwani:011HAEgwIr5E51wJ@cluster0.foqigmu.mongodb.net/doctorsapp?retryWrites=true&w=majority&appName=Cluster0/getinventorydata`, {
        headers: {
          'Authorization': `${token}`
        }
      });
  ...
  const handleAdd = async(index) => {
    ...
    const token = localStorage.getItem('authToken');
    await axios.patch(`mongodb+srv://himitagangwani:011HAEgwIr5E51wJ@cluster0.foqigmu.mongodb.net/doctorsapp?retryWrites=true&w=majority&appName=Cluster0/updateinventory`, updatedInventory[index], {
      headers: {
        'Authorization': `${token}`
      }
    });
  ...
  const handleImport = async(e) => {
    ...
    reader.onload = async (event) => {
      ...
      const token = localStorage.getItem('authToken');
      await axios.post(`mongodb+srv://himitagangwani:011HAEgwIr5E51wJ@cluster0.foqigmu.mongodb.net/doctorsapp?retryWrites=true&w=majority&appName=Cluster0/bulkinventory`, {data:jsonData}, {
        headers: {
          'Authorization': `${token}`
        }
      });
